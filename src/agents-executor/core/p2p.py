    ...
    async def add_mesh(
        ...
        **nats_connect_kwargs,
        ...
        async with self._lock:
            ...
            executor_topic = f"{subject_id}__executor"

            mesh.sub_sid_events = await mesh.nc.subscribe(events_topic, cb=self._mk_on_msg(mesh_id))
            mesh.sub_sid_executor = await mesh.nc.subscribe(executor_topic, cb=self._mk_on_msg(mesh_id))

            mesh.sub_sid = sub_sid
            self._meshes[mesh_id] = mesh

            logger.info(f"[{mesh_id}] Listening on '{self.mesh_subject}' {events_topic} {executor_topic}, HTTP URL saved: {http_url}")
...
def init_p2p_manager(
    *,
    ...
    subject_id = os.getenv("SUBJECT_ID")
    pm = PeersManager(mesh_subject=subject_id, agent_data=agent_data)
